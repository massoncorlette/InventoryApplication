...
async function displayTitles(req, res, next ) {
  ...
  res.render("index", {titles:titles, genres:genres, directors:directors, activeDirector:activeDirector, activeGenre:activeGenre});
};
...
async function displayTitleDetails(req, res, next) {
  ...
  const genres = await db.getAllGenres();
  const directors = await db.getAllDirectors();
  res.render("details", {titleDetails:titleDetails, genres:genres, directors:directors, activeDirector:activeDirector, activeGenre:activeGenre});
};
